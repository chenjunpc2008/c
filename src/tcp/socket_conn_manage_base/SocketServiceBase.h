...
public:
    /*
        ...
        @param[in,out] cid : The id of the connection to shut down.

        @param[in,out] how : A flag that describes what types of operation will no longer be
        ...
        @throw : CIocpException if connection no longer exists.

        CWin32Exception if the IOCP server failed to post this data to the
        IO Completion port.

        @post : If the function completes successfully, the specified operation
        on the socket will no longer be allowed.

        @remark : Shutdown should not be called on the same connection simultaneously from
    ...
    */
    ...
    @param[in,out] cid : The connection to disconnect from.

    @post : If this function completes successfully, the socket will no longer
    be capable of sending or receiving new data (queued data prior to
    disconnect will be sent).

    @remark : If the server is initiating the disconnect, it is recommended to call
