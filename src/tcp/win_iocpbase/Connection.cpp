...
Connection::Connection(SOCKET socket, uint64_t cid, int rcvBufferSize)
    : m_socket(socket), m_id(cid), m_disconnectPending(false), m_sendClosePending(false), m_rcvClosed(false),
      m_rcvContext(m_socket, m_id, IocpContext::Rcv, rcvBufferSize), m_disconnectContext(m_socket, m_id, IocpContext::Disconnect, 0)
{
}
