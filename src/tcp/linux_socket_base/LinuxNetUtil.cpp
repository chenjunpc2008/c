...
namespace LinuxNetUtil
{
    ...
    */
    ...
    @param [out] int& out_errno : 出错时的错误码
    */
    int AcceptConn(const int in_listenfd, int &out_clientfd, string &out_remoteAddr, uint16_t &out_remotePort, int &out_errno)
    {
    ...
    int RecvData(const int sockFd, std::vector<uint8_t> &out_recvData, int &out_errno)
    {
        ...
        while (ret >= 0)
        {
            ...
            if (0 == ret)
            {
                ...
                out_errno = errno;
                if (EINTR == out_errno || 0 == out_errno)
                {
                    return 0;
                }
                else
                {
                    cout << "fd=" << sockFd << "0==ret errno=" << out_errno << " serror=" << strerror(out_errno)
                         << endl;
